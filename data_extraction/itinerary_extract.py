...
def extract_fields(json_data):
    ...
    for segment in segments:
        ...
        aircraft_codes.append(aircraft.get("code", None))
    ...
    aircraft_codes_str = ', '.join(str(code) if code is not None else "" for code in aircraft_codes)
    blacklists_str = ', '.join(str(blacklist) for blacklist in blacklists)

    number_of_stops = max(0, len(segments) - 1)
    
    return duration, iataCodes_str, terminals_str, number_of_stops, departure_durations, arrival_terminals_str, departure_times_str, arrival_times_str, aircraft_codes_str, blacklists_str

# Read CSV file and process each row
with open('flight_search.csv', mode='r') as csv_file:
    csv_reader = csv.DictReader(csv_file)
    output_headers = ['Duration', 'Departure IATACodes', 'Departure Terminals', 'Number of Stops', 'layover durations', 'Arrival Terminals', 'Departure Times', 'Arrival Times', 'Aircraft', 'Blacklist']
    ...
    with open('itinerary.csv', mode='w', newline='') as output_file:
        ...
        for row in csv_reader:
            ...
            duration, departure_iataCodes, departure_terminals, number_of_stops, departure_durations, arrival_terminal, departure_times, arrival_times, aircraft_codes, blacklists = extract_fields(itinerary_data)
            
            writer.writerow([duration, departure_iataCodes, departure_terminals, number_of_stops, departure_durations, arrival_terminal, departure_times, arrival_times, aircraft_codes, blacklists])
